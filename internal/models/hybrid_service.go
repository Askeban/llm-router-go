...
func (h *HybridModelService) GetModels(ctx context.Context) ([]ModelProfile, error) {
	h.mutex.RLock()
	if len(h.cache) > 0 && time.Now().Before(h.cacheExpiry) {
...
func (h *HybridModelService) refreshModels(ctx context.Context) ([]ModelProfile, error) {
	...
	// Step 1: Try to fetch from Analytics AI
	log.Printf("[HYBRID] Fetching models from Analytics AI...")
	analyticsData, err := h.analyticsService.FetchModels()
	if err != nil {
		log.Printf("[HYBRID] Analytics AI fetch failed: %v", err)
		h.analyticsFallbackCount++
	} else {
		log.Printf("[HYBRID] Successfully fetched %d models from Analytics AI", len(analyticsData))
